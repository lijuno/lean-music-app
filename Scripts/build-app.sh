...
APP_PATH="$DIST_ROOT/Lean Music App.app"
CONTENTS_PATH="$APP_PATH/Contents"
MACOS_PATH="$CONTENTS_PATH/MacOS"
RESOURCES_PATH="$CONTENTS_PATH/Resources"
ARM_BUILD_ROOT="$BUILD_ROOT/arm64-release"
INTEL_BUILD_ROOT="$BUILD_ROOT/x86_64-release"
ARM_BINARY="$ARM_BUILD_ROOT/arm64-apple-macosx/release/LeanMusicApp"
INTEL_BINARY="$INTEL_BUILD_ROOT/x86_64-apple-macosx/release/LeanMusicApp"
ENTITLEMENTS_PATH="$PROJECT_ROOT/Packaging/lean-music-app.entitlements"

swift build \
  --package-path "$PROJECT_ROOT" \
  --configuration release \
  --product LeanMusicApp \
...
/usr/bin/lipo \
  ...
  "$ARM_BINARY" \
  "$INTEL_BINARY" \
  -output "$MACOS_PATH/LeanMusicApp"
...
/usr/bin/lipo -info "$MACOS_PATH/LeanMusicApp"
